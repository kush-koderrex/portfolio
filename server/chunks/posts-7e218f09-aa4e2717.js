 ...
 */
function readingTime$2(text, options = {}) {
 ...
 * Export
 */
var readingTime_1 = readingTime$2;
 ...
 * Module dependencies.
 */
const readingTime$1 = readingTime_1;
...
ReadingTimeStream.prototype._transform = function(chunk, encoding, callback) {
  const stats = readingTime$1(chunk.toString(encoding), this.options);
 ...
 * Export
 */
var stream = ReadingTimeStream;

(function (module) {
	module.exports.default = module.exports = readingTime_1;
	module.exports.readingTimeStream = stream;
} (readingTime$3));

var readingTime = /*@__PURE__*/getDefaultExportFromCjs(readingTime$3.exports);

const imports = Object.assign({ "./posts/kivy-python.md": __vite_glob_0_0, "./posts/nao-robot.md": __vite_glob_0_1 });
const posts = [];
...
const allTagsAndPosts = posts.filter((x) => !x.hidden).map((post) => post.tags.map((tag) => {
  return { tag, post };
})).flat();
...
function getRelatedPosts(post) {
  ...
  if (relatedPosts.length === 0) {
    relatedPosts = posts.filter((x) => x.slug !== post.slug).map((x) => ({ post: { ...x, readingTime: readingTime(x.html).text }, count: 0, date: x.date }));
  }
  return relatedPosts;
}
const filteredPosts = posts.filter((post) => !post.hidden).sort((a, b) => new Date(a.date).getTime() > new Date(b.date).getTime() ? -1 : new Date(a.date).getTime() < new Date(b.date).getTime() ? 1 : 0).map((post) => {
...
export { filteredPosts as f };
//# sourceMappingURL=posts-7e218f09-aa4e2717.js.map
